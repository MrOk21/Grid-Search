...
def a_star(tk_root, turtle_canvas):
    rows, cols = 10, 10
    grid = np.zeros((rows, cols), dtype=int)
    
    # Euristica da usare per A*
    euclidean = True  # Da scegliere se si vuole utilizzare la distanza euclidea
    # euclidean = False  # Da scegliere se si vuole utilizzare la distanza di manhattan
    
    # Impostazioni Turtle
    turtle = RawTurtle(turtle_canvas)
    turtle.speed(0)
    ...
    tk_root.bind("<Return>", lambda event: close_window())
    tk_root.mainloop()
